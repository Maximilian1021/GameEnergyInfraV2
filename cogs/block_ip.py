...
from functions.blockIpFunc import remove_ip_from_docker_user_chain
...
class BlockIP(commands.Cog):
    ...
    async def blockIP(self, ctx, message: str):
        try:
            ...
            embed = discord.Embed(title="Ip erfolgreich geblockt", colour=discord.Colour.green(),
                                  description=f"Die IP {message} wurde erfolgreich auf allen Server gesperrt")
    ...
    @commands.has_permissions(ban_members=True)
    @commands.command()
    async def unblockIP(self, ctx, message: str):
        try:
            remove_ip_from_docker_user_chain("server1.max1021.de", "2811", os.getenv('Path_To_SSHKey'), "root", message)
            remove_ip_from_docker_user_chain("server2.max1021.de", "2811", os.getenv('Path_To_SSHKey'), "root", message)
            remove_ip_from_docker_user_chain("server4.max1021.de", "2811", os.getenv('Path_To_SSHKey'), "root", message)
            remove_ip_from_docker_user_chain("server5.max1021.de", "2811", os.getenv('Path_To_SSHKey'), "root", message)
            remove_ip_from_docker_user_chain("server7.max1021.de", "2811", os.getenv('Path_To_SSHKey'), "root", message)
            remove_ip_from_docker_user_chain("bots.max1021.de", "2811", os.getenv('Path_To_SSHKey'), "root", message)
            remove_ip_from_docker_user_chain("ServerR1.game-energy.de", "2811", os.getenv('Path_To_SSHKey'), "root",
                                             message)
            remove_ip_from_docker_user_chain("ServerR2.game-energy.de", "2811", os.getenv('Path_To_SSHKey'), "root",
                                             message)
            remove_ip_from_docker_user_chain("ServerR3.game-energy.de", "2811", os.getenv('Path_To_SSHKey'), "root",
                                             message)

            # Code für erfolgreiche Ausführung
            embed = discord.Embed(title="Ip erfolgreich geblockt", colour=discord.Colour.green(),
                                  description=f"Die IP {message} wurde erfolgreich auf allen Server freigegeben")
            embed.set_footer(text=f"IP Block wurde von {ctx.author} ausgelöst",
                             icon_url="https://cdn.max1021.de/G-E/GameEnergy_Green.png")
            await ctx.send(embed=embed)
